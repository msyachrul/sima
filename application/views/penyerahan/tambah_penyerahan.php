<!-- Page Heading -->
<h1 class="h3 mb-3 text-gray-800">Tambah Data Penyerahan</h1>
        ...
        <div class="card-body">
            <form action="" method="post" autocomplete="off">
                <div class="form-group">
                    ...
                    <input type="date" name="penyerahan_tanggal" id="penyerahan_tanggal" class="form-control <?= form_error('penyerahan_tanggal') ? 'is-invalid' : null ?>" value="<?= set_value('penyerahan_tanggal'); ?>">
                ...
                <div class="form-group">
                    ...
                    <select name="pembelian_id" id="pembelian_id" class="form-control <?= form_error('pembelian_id') ? 'is-invalid' : null; ?>">
                        ...
                        <option value="<?= $p['pembelian_id']; ?>" <?= set_value('pembelian_id') == $p['pembelian_id'] ? 'selected' : null; ?>><?= $p['aset_nama']; ?> - <?= $nomor_pembelian; ?> - Rp<?= number_format($p['pembelian_harga']); ?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select name="user_username" id="user_username" class="form-control <?= form_error('user_username') ? 'is-invalid' : null; ?>">
                        ...
                        <option value="<?= $u['user_username']; ?>" <?= set_value('user_username') == $u['user_username'] ? 'selected' : null; ?>><?= $u['user_nama']; ?></option>
                        <?php endforeach; ?>
                    </select>
